...
const filterNonExistentPathKeys = (config) => {
    return Object.keys(config).reduce((newConfig, nextConfigKey) => (Object.assign(Object.assign({}, newConfig), (fs_extra_1.default.pathExistsSync(config[nextConfigKey].path)
...
const readAssetsDirectory = (options) => {
    ...
    const publicFiles = pathToPosix_1.default(path_1.default.join(assetsDirectory, "public"));
    const staticFiles = pathToPosix_1.default(path_1.default.join(assetsDirectory, "static"));
    const staticPages = pathToPosix_1.default(path_1.default.join(assetsDirectory, "static-pages"));
    const nextData = pathToPosix_1.default(path_1.default.join(assetsDirectory, "_next", "data"));
    const nextStatic = pathToPosix_1.default(path_1.default.join(assetsDirectory, "_next", "static"));
