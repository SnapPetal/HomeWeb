...
const renderRoute = async (event, route, manifest, routesManifest, getPage) => {
    ...
    if (req.url && routesManifest.i18n && !route.isData) {
        req.url = locale_1.addDefaultLocaleToPath(req.url, routesManifest, locale_1.findDomainLocale(req, routesManifest));
    }
...
const handleDefault = async (event, manifest, prerenderManifest, routesManifest, getPage) => {
    ...
    const route = await route_1.routeDefault(request, manifest, prerenderManifest, routesManifest);
    ...
    if (route.isRender) {
        return exports.renderRoute(event, route, manifest, routesManifest, getPage);
    }
