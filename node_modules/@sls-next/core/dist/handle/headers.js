...
const getCustomHeaders = (uri, routesManifest) => {
    const localized = locale_1.addDefaultLocaleToPath(uri, routesManifest);
    const headers = {};
    for (const headerData of routesManifest.headers) {
        if (!match_1.matchPath(localized, headerData.source)) {
            continue;
        }
...
const setCustomHeaders = (event, routesManifest) => {
    ...
    const headers = exports.getCustomHeaders(uri, routesManifest);
