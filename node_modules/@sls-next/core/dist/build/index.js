...
const prepareBuildManifests = async (buildOptions, nextConfig, routesManifest, pagesManifest, prerenderManifest, publicFiles) => {
    ...
    const domainRedirects = normaliseDomainRedirects_1.normaliseDomainRedirects(unnormalisedDomainRedirects);
    const pageManifest = {
        buildId,
        pages: {
            dynamic: [],
        ...
        publicFiles: {},
        trailingSlash: (_a = nextConfig === null || nextConfig === void 0 ? void 0 : nextConfig.trailingSlash) !== null && _a !== void 0 ? _a : false,
        domainRedirects,
        authentication
    };
    const apiManifest = {
    ...
    Object.entries(pagesManifest).forEach(([route, pageFile]) => {
        ...
        if (isHtmlPage(pageFile)) {
            ...
            if (isOtherDynamicRoute) {
                dynamicApi[route] = {
                    file: pageFile,
                    regex: pathToRegexStr_1.pathToRegexStr(route)
                };
            }
            else if (isOptionalCatchAllDynamicRoute) {
                dynamicApi[route] = {
    ...
    Object.entries((_b = prerenderManifest.dynamicRoutes) !== null && _b !== void 0 ? _b : {}).forEach(([route, dynamicSsgRoute]) => {
    ...
    const ssrPages = (pageManifest.pages.ssr = ssr_1.usedSSR(pageManifest, routesManifest, apiPages.dynamic.length > 0 || Object.keys(apiPages.nonDynamic).length > 0));
    ...
    const sortedRoutes = sortedRoutes_1.getSortedRoutes(dynamicRoutes);
    ...
    // Sort api routes
    const sortedApi = sortedRoutes_1.getSortedRoutes(Object.keys(dynamicApi));
