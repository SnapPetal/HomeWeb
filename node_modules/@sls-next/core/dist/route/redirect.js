...
async function getLanguageRedirectPath(req, manifest, routesManifest) {
    ...
    if (req.uri === rootUri && headerCookies) {
        const cookies = cookie_1.parse(headerCookies);
        const nextLocale = cookies["NEXT_LOCALE"];
        if (nextLocale) {
            return await locale_1.getAcceptLanguageLocale(nextLocale, manifest, routesManifest);
        }
    }
    ...
    if (req.uri === rootUri && acceptLanguage) {
        return await locale_1.getAcceptLanguageLocale(acceptLanguage, manifest, routesManifest);
    }
}
...
function getRedirectPath(request, routesManifest) {
    var _a;
    const path = locale_1.addDefaultLocaleToPath(request.uri, routesManifest);
    const redirects = (_a = routesManifest.redirects) !== null && _a !== void 0 ? _a : [];
    for (const redirect of redirects) {
        const match = match_1.matchPath(path, redirect.source);
        if (match) {
            const compiledDestination = match_1.compileDestination(redirect.destination, match.params);
